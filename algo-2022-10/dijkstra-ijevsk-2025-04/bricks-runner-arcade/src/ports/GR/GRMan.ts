...
export class GRMan {
    ...
    draw = () => {
        ...
        if (this.manAni === ManAni.PIPE_STAND) {
            sprite = pipeMan;
        }
        if (this.manAni === ManAni.PIPE_LEFT) {
            const frames = [pipeMan, pipeManL];
            sprite = frames[this.counter % 2];
        }
        if (this.manAni === ManAni.PIPE_RIGHT) {
            const frames = [pipeMan, pipeManR];
            sprite = frames[this.counter % 2];
        }
